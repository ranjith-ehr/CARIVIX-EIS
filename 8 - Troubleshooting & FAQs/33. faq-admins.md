...
**How do I add or update a geofence zone?**

1. Go to **Admin Console → Geofences**.
2. To add: click **Add Zone**, enter the zone name, choose **Radius** or **Polygon**, define the boundary, assign to departments or all employees, and click **Save Zone**.
3. To update: find the zone in the list, click **Edit**, modify the boundary or assignment, and click **Save Changes**.

Changes take effect immediately for all subsequent check-ins. Existing records are not retroactively updated.

---

**What is the buffer tolerance and when should I change it?**

The buffer tolerance adds a margin (default: 20 meters) around the geofence boundary to account for GPS drift indoors. If employees who are physically present are consistently flagged as outside the zone, increase the tolerance. If you are seeing successful check-ins from employees who are clearly not in the office, reduce the tolerance.

To adjust: go to **Admin Console → Geofences → Edit Zone → Buffer Tolerance (meters)**.

---

**Can I assign different geofence zones to different employees?**

Yes. When creating or editing a zone, you can assign it to specific departments or to individual employees. An employee assigned to multiple zones can check in successfully from any of them.
...
1. Go to **Admin Console → Users** and search for the employee.
2. Open their profile and click **Reset Face Enrollment**.
3. Confirm the action. The employee will receive an email notification with instructions to re-enroll.

The employee must complete re-enrollment before they can check in again.
